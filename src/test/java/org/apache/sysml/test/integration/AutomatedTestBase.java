...
public abstract class AutomatedTestBase {
	...
	protected void runRScript(boolean newWay) {
		...
		if (!newWay) {
			...
			// *** LOCAL HACK ***
			// Rscript is not found on a local machine. Therefore specifing the
			// exact location here.
			// TO BE REMOVED before committing to master
			cmd = rCmd.replaceFirst("Rscript", "/usr/local/bin/Rscript");
			// *** END LOCAL HACK ***
		}
